...
| A tenant admin escalates to platform-operator (superadmin) scope | `require_superadmin_auth()` gates `/api/tenants*`; a tenant-scoped admin passing the plain admin role check is explicitly insufficient there (a tenant admin could otherwise move itself into the default tenant and become a de facto superadmin). **Account management is tenant-scoped too** *(v6.4.3)* — until then that gate had an open side door: `POST /api/users` stamped no tenant, so an account created by a tenant admin landed in `default` and was itself a superadmin, and `PATCH`/`DELETE`/`GET /api/users` reached every tenant's accounts. A new account now inherits its creator's tenant (the same fix API keys got in v6.1.1), and a tenant admin sees and manages only its own. |
