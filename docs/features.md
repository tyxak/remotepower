...
| WordPress recent logins | The WordPress connector shows the **last 5 successful logins** — timestamp, account, source IP, and country when a GeoIP MMDB is configured — via the Simple History plugin's REST API (Application-password auth); without the plugin it still monitors REST reachability *(v6.4.0)* |
