...
| Alert mute & tuning | Per-(host, event) **mute** silences one exact alert from one asset (inbox + webhook + needs-attention) while history keeps recording; a mute now also drops the signal from Needs-Attention, so it **stops depressing the host's and the fleet's health score** — matched per event *and* severity, so muting `av_warning` never hides a critical `av_infected` *(health fix v6.1.2)*; the Alerts/dashboard Ack button is an **X mute**; Monitoring → Tuning ranks the noisiest alerts/sources from the timeline. `GET/POST /api/alert-mutes`, `GET /api/alert-tuning` *(v5.6.0)* |
