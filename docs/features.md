...
| Thermal | Hottest hosts, per-sensor expand, ~24h trend sparkline, per-host warning/critical thresholds *(v4.0.0)*; expanding a host also draws a long-range **temperature timeline** — min/avg/max per point over 24 hours, 8 days, 30 days or 2 years, from hourly-folded roll-ups. `GET /api/devices/{id}/thermal/rollup` *(v6.4.2)* |
...
| Wayfinding strings resolve | Every "Group → Page" pointer in a hint, empty state or toast names a real nav page or Settings tab, checked by a test that derives the destination set from the markup itself. `Settings → Proxmox` (the tab has read Virtualization since v5.6.0), `Monitoring → GPUs` (it is under Hardware), `Admin → CMDB` (it is under Fleet), `Settings → Alerts`, `Settings → Log rules`, `Settings → Service watch` and `Settings → Power` all pointed at places that do not exist *(v6.4.2)* |
