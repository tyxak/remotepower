...
| Agentic alert triage | The model investigates one alert through read-only, device-scoped evidence tools (journal tail, log search, sibling alerts, recent commands, latest sweep) under a strict-JSON protocol with a hard tool budget, then stores a verdict + evidence trail on the alert ("AI verdict" badge); `POST /api/alerts/{id}/ai-triage` *(v6.3.1)* |
