...
| Check catalog | Server-evaluated process/port-open/port-closed; host-evaluated **systemd unit active** *(v5.6.0)*, **Windows service running** *(v6.1.3)*, file present/absent, job freshness, log error rate; **~100 ready-made templates** that pre-fill the form — incl. a RemotePower self-infra set, a Windows section (Windows Update / Defender / Firewall / RDP / WinRM / IIS / SQL Server / AD / Hyper-V / Exchange), and cross-platform **database up?** checks (Postgres / MySQL / MSSQL / Redis / Mongo / Oracle, port-based so one check covers either OS); host target is a device-search typeahead; a unit/service check can also be added to the device's Services watch-list; editable *(catalog v5.6.0)* |
| Windows posture checks | First-class Checks rows from the Windows agent: **Defender real-time protection** (off = critical), **Defender signature age**, **BitLocker** on the OS volume, **Windows Firewall** per profile, and the **Windows Update service**. The CPU check renders from `cpu_percent` where there's no loadavg. *(v6.1.3)* |
...
| Privileged-group tripwire | Someone landing in `sudo`/`wheel` (Linux) or `Administrators` (Windows) raises an edge-triggered `priv_group_added` — the classic post-compromise persistence step, and the classic nobody-told-me change. Both agents already reported the membership; nothing ever diffed it. First contact baselines silently; removals don't fire (crying wolf gets the event muted, and a muted event catches nothing) *(v6.1.3)* |
| USB device tripwire | A USB device appearing on a host is a physical-access signal → `usb_device_added`. Read from sysfs (works in the containerized agent, where `lsusb` isn't installed); keyed by VID:PID so the same stick in another port doesn't re-fire. Detection only — peripheral *enforcement* is deliberately out of scope. Lands in the inbox but doesn't page by default *(v6.1.3)* |
