...
| Security Advisory | **What to fix first** — one prioritized, cross-layer list (application / exposure / OS / identity / integrity) built from data already collected, at fleet, group, tag or host scope. Each entry carries why it matters, the concrete action and the evidence; identical findings across hosts group into one decision. An **AI option** sends a server-side-redacted brief (titles/severities/host counts only — never evidence) for ordering and chain-spotting. `GET /api/security/advisory` *(v6.4.0)* |
