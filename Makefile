...
help:
	...
	@echo ""
	@echo " Iterate"
	@echo "  make test-fast   - whole suite via xdist (~2m, e2e excluded) — for iterating"
	@echo "  make test        - full suite, JSON backend, serial (what CI runs)"
	@echo "  make test-sqlite - full suite, SQLite backend"
	@echo "  make test-both   - JSON + SQLite (background them instead to halve wall-clock)"
	...
	@echo "  make check       - test-both + lint (the everyday gate)"
	@echo "  make bandit      - bandit against the committed baseline (expect 0 new)"
	@echo "  make codeql      - local CodeQL, honouring .github/codeql/codeql-config.yml"
	@echo "  make bandit-baseline - regenerate the bandit baseline (after triaging)"
	@echo "  make ci-parity   - reproduce prod CI exactly (Python 3.14, ci.yml dep list)"
	@echo ""
	@echo " Release"
	@echo "  make pre-release - THE PRE-TAG GATE. check + test-pg + dist + ci-parity +"
	@echo "                     CodeQL, and writes the .pre-release-ok push stamp the"
	@echo "                     pre-push hook requires for a production push."
	@echo "  make dist        - build dist/$(DIST_NAME).tar.gz and run the suite against it"
	@echo "  make release     - dist + a detached GPG signature, and print the gh line"
	@echo "  make bump        - version-bump helper"
	@echo "  make sbom-self   - CycloneDX SBOM for this tree"
	...
	@echo "  make install-dev - install the dev toolchain locally"
	@echo "  make scan-demo   - drive a security scan to completion (needs a running server)"
	@echo "  make postman / make sdk - export the API collection / client SDK"
