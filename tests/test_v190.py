...
class TestVaultEndpoints(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for name in ('USERS_FILE', 'TOKENS_FILE', 'DEVICES_FILE',
                     'CMDB_FILE', 'CMDB_VAULT_FILE', 'AUDIT_LOG_FILE',
                     'CONFIG_FILE'):
    ...
    def test_setup_then_unlock_then_status(self):
        user, token = _seed_admin()
        _set_request('POST', '/api/cmdb/vault/setup',
        ...
        _set_request('POST', '/api/cmdb/vault/unlock',
                     body={'passphrase': 'Strong-Pass-1234'},
                     headers=_auth_headers(token))
    ...
    def test_unlock_with_wrong_passphrase_fails_and_audits(self):
        ...
        actions = [e.get('action') for e in (log if isinstance(log, list) else log.get('entries', []))]
...
class TestAssetCrud(unittest.TestCase):
    ...
    def test_update_sets_all_fields(self):
        _set_request('PUT', f'/api/cmdb/{self.dev}',
                     body={
    ...
    def test_update_404_on_unknown_device(self):
        _set_request('PUT', '/api/cmdb/no-such-device',
    ...
    def test_search_filters_by_query(self):
        ...
        devices = api.load(api.DEVICES_FILE)
        devices['dev-other'] = {
            'name': 'db-host', 'hostname': 'db-host', 'os': 'Debian 12',
            'ip': '10.0.0.6', 'mac': '11:22:33:44:55:66', 'token': 't',
...
class TestCredentials(unittest.TestCase):
    ...
    def _hdrs(self):
        return {**_auth_headers(self.token),
                'HTTP_X_RP_VAULT_KEY': self.key_hex}

    def test_add_credential_succeeds(self):
        _set_request('POST', f'/api/cmdb/{self.dev}/credentials',
                     body={'label': 'root', 'username': 'root',
                           'password': 'hunter2'},
                     headers=self._hdrs())
    ...
    def test_list_returns_metadata_no_ciphertext(self):
        ...
        _set_request('GET', f'/api/cmdb/{self.dev}/credentials',
                     headers=_auth_headers(self.token))
    ...
    def test_delete_credential(self):
        ...
        _set_request('DELETE',
                     f'/api/cmdb/{self.dev}/credentials/{cid}',
                     headers=_auth_headers(self.token))
    ...
    def test_passphrase_rotation_re_encrypts_creds(self):
        ...
        # Rotate
        _set_request('POST', '/api/cmdb/vault/change',
                     body={'old_passphrase': 'Strong-Pass-1234',
                           'new_passphrase': 'Other-Pass-9999'},
                     headers=_auth_headers(self.token))
        ...
        _set_request('POST',
                     f'/api/cmdb/{self.dev}/credentials/{cid}/reveal',
                     headers={**_auth_headers(self.token),
                              'HTTP_X_RP_VAULT_KEY': new_key_hex})
        s, b = _call(api.handle_cmdb_credentials_reveal, self.dev, cid)
        self.assertEqual(s, 200)
...
class TestServerFunctions(unittest.TestCase):
    ...
    def test_distinct_values_returned_sorted(self):
        ...
        api.save(api.CMDB_FILE, cmdb)
        _set_request('GET', '/api/cmdb/server-functions',
                     headers=_auth_headers(self.token))
