...
class TestHeartbeatWiring(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.hb = api[idx: idx + 95000]   # widened as handle_heartbeat grew (v3.13.0 CMDB hardware fields; v3.14.0 network_io + secrets ingest; v4.1.0 inode/fd/conntrack + clock/gateway/oom edge-triggers; v4.2.0 sweep mailq/pkg_scan_ts + reinstall-audit hoist; v4.10.0 agent-stopping branch + backup_verify ingest; v5.4.1 device-token migration block; v5.5.0 failed_unit edge-trigger; v5.8.0 W5-1 LLDP-neighbor ingest)
