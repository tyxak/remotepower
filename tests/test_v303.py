...
# ─── Version pins (loosened — v3.0.4 holds the strict pins now) ─────────────
#
# This block originally asserted `version == '3.0.3'` exactly. Once
...
class TestVersionBumps(unittest.TestCase):
    ...
    def test_v303_entry_still_present(self):
        """The v3.0.3 entry must still appear somewhere in CHANGELOG.md —
        loosened from "must be the top entry" now that v3.0.4 has shipped."""
        chlog = (REPO_ROOT / 'CHANGELOG.md').read_text()
        self.assertIn('## v3.0.3', chlog,
            'v3.0.3 entry was removed from CHANGELOG.md')
