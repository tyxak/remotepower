...
# Syslog receiver (v6.3.0): same copied-binary pattern as webterm. Its unit
# is STATIC (DynamicUser) so refreshing it is safe, unlike the rendered ones.
if [[ -f /usr/local/bin/remotepower-syslogd ]]; then
    install -m 755 "$SCRIPT_DIR/server/syslog/remotepower-syslogd.py" \
        /usr/local/bin/remotepower-syslogd
    if [[ -f "$SCRIPT_DIR/packaging/remotepower-syslogd.service" \
          && -f /etc/systemd/system/remotepower-syslogd.service ]]; then
        install -m 644 "$SCRIPT_DIR/packaging/remotepower-syslogd.service" \
            /etc/systemd/system/remotepower-syslogd.service
        systemctl daemon-reload
    fi
    if systemctl is-active --quiet remotepower-syslogd 2>/dev/null; then
        systemctl restart remotepower-syslogd
        info "remotepower-syslogd refreshed + restarted (service was active)"
    fi
fi
# NetFlow/IPFIX receiver (v6.3.1): same static-unit pattern as syslogd. The
# daemon imports flow_parse.py from its own dir, so install both side by side.
if [[ -f /usr/local/bin/remotepower-flowd ]]; then
    install -m 755 "$SCRIPT_DIR/server/flow/remotepower-flowd.py" \
        /usr/local/bin/remotepower-flowd
    install -m 644 "$SCRIPT_DIR/server/flow/flow_parse.py" \
        /usr/local/bin/flow_parse.py
    if [[ -f "$SCRIPT_DIR/packaging/remotepower-flowd.service" \
          && -f /etc/systemd/system/remotepower-flowd.service ]]; then
        install -m 644 "$SCRIPT_DIR/packaging/remotepower-flowd.service" \
            /etc/systemd/system/remotepower-flowd.service
        systemctl daemon-reload
    fi
    if systemctl is-active --quiet remotepower-flowd 2>/dev/null; then
        systemctl restart remotepower-flowd
        info "remotepower-flowd refreshed + restarted (service was active)"
    fi
fi
# KMIP key server (v6.4.0): same static-unit pattern. The shared secret lives in
# /etc/remotepower/kmipd.env and is NEVER touched here — refreshing code must
# not rotate a secret the API is verifying against.
if [[ -f /usr/local/bin/remotepower-kmipd ]]; then
    install -m 755 "$SCRIPT_DIR/server/kmip/remotepower-kmipd.py" \
        /usr/local/bin/remotepower-kmipd
    if [[ -f "$SCRIPT_DIR/packaging/remotepower-kmipd.service" \
          && -f /etc/systemd/system/remotepower-kmipd.service ]]; then
        install -m 644 "$SCRIPT_DIR/packaging/remotepower-kmipd.service" \
            /etc/systemd/system/remotepower-kmipd.service
        systemctl daemon-reload
    fi
    if systemctl is-active --quiet remotepower-kmipd 2>/dev/null; then
        systemctl restart remotepower-kmipd
        info "remotepower-kmipd refreshed + restarted (service was active)"
    fi
fi
