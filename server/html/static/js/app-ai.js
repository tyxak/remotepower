...
async function loadAISettings() {
  ...
  _setSrc('ai-rag-src-sudolog',          rs.sudo_log !== false);
  _setSrc('ai-rag-src-selfobs',          rs.self_obs !== false);
  _setSrc('ai-rag-src-inventory',        rs.inventory !== false);
  ...
  const payload = {
    ...
    rag: {
      ...
      sources: {
        ...
        sudo_log:          !!document.getElementById('ai-rag-src-sudolog')?.checked,
        self_obs:          !!document.getElementById('ai-rag-src-selfobs')?.checked,
        inventory:         !!document.getElementById('ai-rag-src-inventory')?.checked,
      },
      history_limits: {
...
const AI_INSIGHTS = [
  ...
  // v6.3.1: the Security Advisory's AI option. The page posts the REDACTED
  // brief as the message, so this card is the same advisor reached from the hub
  // with no scope preloaded.
  { key: 'security_advisory',   cat: 'advisors',  label: 'Security advisory',          desc: 'What to fix first across OS, exposure, identity, integrity and applications.', msg: "Review the fleet's security posture and tell me what to fix first, in order, and why." },
  ...
  { key: 'patch_priorities',    cat: 'planning',  label: 'Patch prioritisation',        desc: 'Which pending updates to apply first across the fleet, by risk.', msg: "Prioritise the fleet's pending package updates by risk — security-flagged and CVE-fixing updates first — and note which hosts need a reboot." },
