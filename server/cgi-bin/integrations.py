...
class HTTPClient:
    ...
    def _full(self, path, params=None):
        url = path if path.startswith(('http://', 'https://')) else self.base + (
...
def _register(type_, label, category, fields, notes=''):
    ...
    def deco(fn):
        CONNECTORS[type_] = {
            'type': type_, 'label': label, 'category': category,
            'fields': fields, 'notes': notes, 'health': fn,
        }
        return fn
    return deco
...
def list_connectors():
    ...
    for c in CONNECTORS.values():
        out.append({'type': c['type'], 'label': c['label'], 'category': c['category'],
                    'fields': c['fields'], 'notes': c['notes']})
...
def _pihole(inst, c):
    ...
    try:
        ...
        ver = (((v.get('version') or {}).get('core') or {}).get('local') or {}).get('version', '') or ''
    except IntegrationError:
        pass
    return {'status': OK, 'version': ver,
            'detail': f"{int(_num(total))} queries today, {round(_num(blocked_pct),1)}% blocked",
            'metrics': {'queries_today': total, 'blocked_pct': blocked_pct,
                        'domains_blocked': g.get('domains_being_blocked', 0)}}


@_register('adguard', 'AdGuard Home', 'dns',
...
def _adguard(inst, c):
    ...
    return {'status': status, 'version': ver, 'detail': detail,
            'metrics': {'queries': queries, 'blocked': blocked, 'protection': prot}}


@_register('truenas', 'TrueNAS', 'storage',
...
def _truenas(inst, c):
    ...
    bad = [p.get('name', '?') for p in (pools or []) if str(p.get('status', '')).upper() not in ('ONLINE', 'HEALTHY')]
    crit = warn = 0
    try:
    ...
    return {'status': status, 'version': ver, 'detail': detail,
            'metrics': {'pools': len(pools or []), 'pools_bad': len(bad),
                        'alerts_crit': crit, 'alerts_warn': warn}}


@_register('unifi', 'UniFi Network', 'network',
           [_field('username', 'Username', TEXT), _field('secret', 'Password', PASSWORD),
...
def _unifi(inst, c):
    ...
    c.post_json('/api/login', {'username': inst.get('username', ''),
                               'password': inst.get('secret', ''), 'remember': True})
    ...
    detail = (f"{len(subs)} subsystems OK" if not bad
              else f"degraded: {', '.join(bad)}")
    return {'status': status, 'detail': detail,
            'metrics': {'subsystems': len(subs), 'subsystems_bad': len(bad)}}


@_register('homeassistant', 'Home Assistant', 'apps',
...
def _homeassistant(inst, c):
    ...
    return {'status': status, 'version': ver,
...
@_register('pbs', 'Proxmox Backup Server', 'backup',
           [_field('username', 'Token ID (user@realm!name)', TEXT), _field('secret', 'Token secret', PASSWORD)],
...
def _pbs(inst, c):
    ...
    ver = ''
    try:
        ver = ((c.get_json('/api2/json/version', headers=h) or {}).get('data') or {}).get('version', '')
    except IntegrationError:
        pass
    return {'status': status, 'version': ver,
...
@_register('kubernetes', 'Kubernetes / k3s', 'orchestration',
...
def _kubernetes(inst, c):
    ...
    for n in (nodes or {}).get('items', []):
        ready = next((cd for cd in (n.get('status', {}).get('conditions') or []) if cd.get('type') == 'Ready'), None)
    ...
    detail += f" · {running} running" + (f", {crash} crashloop" if crash else "") + (f", {pending} pending" if pending else "")
    return {'status': status, 'detail': detail,
            'metrics': {'nodes_notready': len(not_ready), 'pods_crashloop': crash, 'pods_pending': pending}}


@_register('vcenter', 'VMware vCenter / ESXi', 'orchestration',
           [_field('username', 'Username', TEXT), _field('secret', 'Password', PASSWORD)],
...
def _vcenter(inst, c):
    ...
    try:
        ...
        pass
    down = [hh.get('name', '?') for hh in (hosts or []) if str(hh.get('connection_state', 'CONNECTED')).upper() != 'CONNECTED']
    status = CRIT if down else OK
    poweredon = sum(1 for v in vms if str(v.get('power_state', '')).upper() == 'POWERED_ON')
    return {'status': status,
            'detail': f"{len(hosts or [])} hosts" + (f", disconnected: {', '.join(down)}" if down else "")
                      + f" · {poweredon}/{len(vms)} VMs on",
            'metrics': {'hosts': len(hosts or []), 'hosts_down': len(down),
                        'vms': len(vms), 'vms_on': poweredon}}


@_register('unraid', 'Unraid', 'storage',
           [_field('secret', 'API key', PASSWORD)],
...
def _unraid(inst, c):
    ...
    return {'status': status, 'detail': f"array {state.lower() or 'unknown'}",
...
@_register('traefik', 'Traefik', 'proxy',
...
@_register('npm', 'Nginx Proxy Manager', 'proxy',
...
def _npm(inst, c):
    tok = (c.post_json('/api/tokens', {'identity': inst.get('username', ''),
                                       'secret': inst.get('secret', '')}).json() or {}).get('token', '')
    if not tok:
...
@_register('uptimekuma', 'Uptime Kuma', 'observability',
...
def _uptimekuma(inst, c):
    ...
    for _mid, lst in beats.items():
        ...
        if int((lst[-1] or {}).get('status', 1)) == 0:
            down += 1
    status = CRIT if down else OK
    return {'status': status, 'detail': f"{total} monitors, {down} down",
            'metrics': {'monitors': total, 'down': down}}


@_register('netdata', 'Netdata', 'observability',
...
def _netdata(inst, c):
    ...
    return {'status': status, 'version': ver,
            'detail': (f"{crit} critical, {warn} warning alarms" if (crit or warn) else "no active alarms"),
            'metrics': {'alarms_critical': crit, 'alarms_warning': warn}}


@_register('grafana', 'Grafana', 'observability',
...
def _grafana(inst, c):
    ...
    return {'status': status, 'version': ver,
            'detail': f"database {db or 'unknown'}", 'metrics': {'database_ok': db == 'ok'}}
...
@_register('jellyfin', 'Jellyfin', 'media',
           [_field('secret', 'API key', PASSWORD)],
...
def _plex(inst, c):
    ...
    try:
        ...
        pass
    return {'status': OK, 'version': ver, 'detail': f"up · {active} active sessions",
            'metrics': {'sessions_active': active}}


@_register('nextcloud', 'Nextcloud', 'apps',
...
def _nextcloud(inst, c):
    ...
    info = c.get_json('/ocs/v2.php/apps/serverinfo/api/v1/info', headers=h, params={'format': 'json'})
    ...
    update_available = bool(system.get('update', {}).get('available')) if isinstance(system.get('update'), dict) else False
...
@_register('sabnzbd', 'SABnzbd', 'download',
           [_field('secret', 'API key', PASSWORD)],
           notes='SABnzbd API key (Config → General).')
def _sabnzbd(inst, c):
    q = c.get_json('/api', params={'mode': 'queue', 'output': 'json', 'apikey': inst.get('secret', '')})
...
def _qbittorrent(inst, c):
    ...
    return {'status': status, 'detail': f"{count} torrents, {dl} KB/s, {conn}",
            'metrics': {'torrents': count, 'download_kbs': dl, 'connection': conn}}


@_register('transmission', 'Transmission', 'download',
...
def _transmission(inst, c):
    ...
    if r.status == 409:
        sid = r.headers.get('X-Transmission-Session-Id') or r.headers.get('x-transmission-session-id')
    ...
    return {'status': OK, 'detail': f"{total} torrents, {active} active, {dl} KB/s",
            'metrics': {'torrents': total, 'active': active, 'download_kbs': dl}}


@_register('deluge', 'Deluge', 'download',
           [_field('secret', 'WebUI password', PASSWORD)],
           notes='Deluge Web JSON API — logs in for a session cookie.')
def _deluge(inst, c):
    auth = c.post_json('/json', {'method': 'auth.login', 'params': [inst.get('secret', '')], 'id': 1}).json()
    if not (auth or {}).get('result'):
        raise IntegrationError('login failed')
    ui = c.post_json('/json', {'method': 'web.update_ui', 'params': [['name', 'state'], {}], 'id': 2}).json()
    res = (ui or {}).get('result') or {}
    torrents = res.get('torrents') or {}
    stats = res.get('stats') or {}
    dl = int(_num(stats.get('download_rate', 0))) // 1024
    return {'status': OK, 'detail': f"{len(torrents)} torrents, {dl} KB/s",
            'metrics': {'torrents': len(torrents), 'download_kbs': dl}}


@_register('servarr', 'Servarr (Sonarr / Radarr / Prowlarr / Lidarr)', 'download',
           [_field('secret', 'API key', PASSWORD)],
...
@_register('overseerr', 'Overseerr / Jellyseerr', 'request',
           [_field('secret', 'API key', PASSWORD)],
...
def _overseerr(inst, c):
    ...
    pending = 0
    try:
        pending = int(_num((c.get_json('/api/v1/request/count', headers=h) or {}).get('pending', 0)))
    except IntegrationError:
        pass
