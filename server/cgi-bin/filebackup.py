...
def build_backup_command(spec, job_id):
    ...
    pre = (
        f"mkdir -p {shlex.quote(mnt)} && {_mount_cmd(dest, mnt)} && mkdir -p {shlex.quote(dstdir)}"
    )
    ...
    if method == "rsync":
        ...
        xfer = (
            f'tar czf {shlex.quote(dstdir)}/"{job_id}-$(date +%Y%m%d-%H%M%S).tar.gz" ' f"{qpaths}"
        )
...
def build_list_command(spec, job_id):
    ...
    if transport == "ssh":
        remote = shlex.quote(f'{dest["user"]}@{dest["host"]}')
        return f'{_ssh_e(dest.get("port", 22))} {remote} ' f'"ls -1t {rpath} 2>/dev/null | {grep}"'
    mnt = f"{_MNT_BASE}/{job_id}-ls"
    srcdir = f'{mnt}/{rpath.lstrip("/")}'
    pre = f"mkdir -p {shlex.quote(mnt)} && {_mount_cmd(dest, mnt)}"
    post = f"umount {shlex.quote(mnt)}; rmdir {shlex.quote(mnt)} 2>/dev/null || true"
    return (
        f"{pre} && {{ ls -1t {shlex.quote(srcdir)} 2>/dev/null | {grep}; rc=$?; "
