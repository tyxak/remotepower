...
if _AVAILABLE:
    ...
    # v6.4.2 fleet-ops. Every field is OPTIONAL with a loose `mode='before'`
    # coercer and NO ge/le/min_length: these are ADDITIVE SUPERSET pre-checks,
    # so a body the handler would have accepted must still validate here. The
    # handlers keep their own sanitisers/clamps (which is where the real
    # validation lives) and each model accepts an empty {} body.
    class DevicesBulkAttrsRequest(BaseModel):
        model_config = ConfigDict(extra='ignore', protected_namespaces=())
        ...
        update_channel: str = ''
        _v0 = field_validator('decommissioned', 'monitored', mode='before')(_coerce_bool_loose)
        _v1 = field_validator('group', 'icon', 'notes', 'offline_alert_delay_min',
                              'poll_interval', 'reachability', 'site',
                              'update_channel', mode='before')(_coerce_str_loose)

    class TaxonomyGroupOpRequest(BaseModel):
        model_config = ConfigDict(extra='ignore', protected_namespaces=())
        # `from` is a Python keyword, so it can only arrive through extra
        # (=ignore) — the handler reads it off the raw body dict, which is
        # exactly what _read_valid returns. Modelled here for the OTHER keys.
        group: str = ''
        to: str = ''
        _v0 = field_validator('group', 'to', mode='before')(_coerce_str_loose)

    class PatchApprovalSetRequest(BaseModel):
        model_config = ConfigDict(extra='ignore', protected_namespaces=())
        note: str = ''
        package: Any = None
        packages: Any = None
        state: str = ''
        _v0 = field_validator('note', 'state', mode='before')(_coerce_str_loose)

    class PatchApprovalClearRequest(BaseModel):
        model_config = ConfigDict(extra='ignore', protected_namespaces=())
        package: Any = None
        packages: Any = None
    ...
    class ConfigSaveRequest(BaseModel):
        ...
        dependency_link_alerts: bool = False   # v6.3.1: flow-verified depends_on alerting
        dependency_silence_min: str = ''
        ...
        flow_export_silence_seconds: str = ''
        flow_export_check_seconds: str = ''
        health_grade_good: str = ''
        health_grade_fair: str = ''
        health_grade_poor: str = ''
        risk_level_critical: str = ''
        risk_level_high: str = ''
        risk_level_medium: str = ''
        reliability_crit: str = ''
        reliability_high: str = ''
        reliability_medium: str = ''
        # v6.2.2 batch 1: alert-firing thresholds (loose str, coerced below).
        inode_warn_percent: str = ''
        inode_crit_percent: str = ''
        fd_warn_percent: str = ''
        fd_crit_percent: str = ''
        conntrack_warn_percent: str = ''
        conntrack_crit_percent: str = ''
        offline_missed_polls: str = ''
        laptop_offline_grace_hours: str = ''
        resolver_failures_before_alert: str = ''
        ip_rep_confirm_scans: str = ''
        tls_warn_days: str = ''
        tls_crit_days: str = ''
        cert_file_expiring_days: str = ''
        contract_warn_days: str = ''
        contract_soon_days: str = ''
        os_eol_soon_days: str = ''
        av_sig_stale_days: str = ''
        gateway_latency_high_ms: str = ''   # v6.4.0
        battery_health_low_pct: str = ''    # v6.4.0
        # v6.2.2 batch 3: hardware/thermal/SMART thresholds (loose str, coerced below).
        thermal_hot_c: str = ''
        thermal_crit_c: str = ''
        gpu_hot_c: str = ''
        smart_wear_warn_pct: str = ''
        smart_wear_high_pct: str = ''
        smart_realloc_crit_sectors: str = ''
        disk_predict_medium_eta_days: str = ''
        ecc_error_alert_min_delta: str = ''
        # v6.2.2 batch 5: final threshold sweep (loose str, coerced below).
        disk_forecast_crit_days: str = ''
        disk_forecast_warn_days: str = ''
        oom_recent_window_seconds: str = ''
        defender_sig_warn_days: str = ''
        cpu_pct_warn: str = ''
        cpu_pct_crit: str = ''
        patch_na_warn_count: str = ''
        container_restart_delta_threshold: str = ''
        container_restarting_min: str = ''
        attention_event_ttl_hours: str = ''
        after_hours_na_window_hours: str = ''
        snmp_cpu_warn_percent: str = ''
        snmp_cpu_crit_percent: str = ''
        agentless_ping_fail_threshold: str = ''
        ct_fail_backoff: str = ''
        github_new_issue_per_poll_cap: str = ''
        self_high_load_ratio: str = ''
        self_mem_warn_pct: str = ''
        self_online_pct_warn: str = ''
        compliance_tls_expiring_days: str = ''
        compliance_lookback_days: str = ''
        cis_disk_pct_max: str = ''
        cis_swap_pct_max: str = ''
        dmarc_pct_min: str = ''
        proxmox_backup_warn_days: str = ''
        metric_recovery_buffer: str = ''
        min_online_ttl: str = ''
        unstable_host_returns_min: str = ''
        unstable_host_window_days: str = ''
        reliability_reboot_churn_min: str = ''
        reliability_wear_high_pct: str = ''
        monitor_history_max: str = ''
        command_history_max: str = ''
        # v6.2.2 batch 6: FLOAT thresholds (loose str, coerced to float in the
        # handler's parallel float loop — NOT the int loop).
        forecast_min_r2: str = ''
        forecast_ceiling_memory: str = ''
        forecast_floor_memory: str = ''
        forecast_ceiling_swap: str = ''
        forecast_floor_swap: str = ''
        forecast_ceiling_load: str = ''
        forecast_floor_load: str = ''
        reliability_realloc_growth_per_day: str = ''
        reliability_health_decline_per_day: str = ''
        cvss_band_critical: str = ''
        cvss_band_high: str = ''
        cvss_band_medium: str = ''
        # v6.2.2 batch 4: per-factor score weights (loose str, coerced below).
        health_weight_critical: str = ''
        health_weight_warning: str = ''
        health_weight_info: str = ''
        risk_weight_offline: str = ''
        risk_weight_cve_critical: str = ''
        risk_weight_cve_high: str = ''
        risk_weight_pending_updates: str = ''
        risk_weight_exposed_world: str = ''
        risk_weight_policy_violation: str = ''
        risk_weight_expiry_expired: str = ''
        risk_weight_expiry_soon: str = ''
        risk_weight_mount_issue: str = ''
        risk_weight_reboot_required: str = ''
        risk_weight_firewall_off: str = ''
        risk_weight_storage_degraded: str = ''
        risk_weight_smart_failure: str = ''
        risk_weight_kernel_outdated: str = ''
        risk_weight_failed_units: str = ''
        risk_weight_os_eol: str = ''
        risk_weight_os_eol_soon: str = ''
        risk_weight_overheating: str = ''
        risk_weight_config_drift: str = ''
        risk_weight_clock_skew: str = ''
        risk_weight_gateway_down: str = ''
        risk_weight_oom_recent: str = ''
        risk_weight_cve_kev: str = ''
        risk_weight_av_bad: str = ''
        risk_weight_image_cves: str = ''
        risk_weight_backup_stale: str = ''
        risk_weight_secrets_exposed: str = ''
        risk_weight_patch_sla_breach: str = ''
        reliability_weight_smart_failing: str = ''
        reliability_weight_realloc_growing: str = ''
        reliability_weight_pending_sectors: str = ''
        reliability_weight_wear_high: str = ''
        reliability_weight_spare_low: str = ''
        reliability_weight_ecc_uncorrectable: str = ''
        reliability_weight_ecc_correctable: str = ''
        reliability_weight_reboot_churn: str = ''
        reliability_weight_health_declining: str = ''
        reliability_weight_overheating: str = ''
        reliability_weight_oom_recent: str = ''
        ...
        log_buffer_max_bytes_per_unit: str = ''   # v6.4.2
        log_buffer_retention_hours: str = ''      # v6.4.2
        ...
        wol_broadcast: str = ''
        wol_port: str = ''
        _v0 = field_validator('ack_comment_enabled', 'agentless_ssh_enabled', 'alert_email_ack_links', 'allow_internal_monitors', 'audit_forward_enabled', 'audit_forward_tcp', 'billing_enabled', 'brute_force_enabled', 'cert_expiry_alerts_enabled', 'change_approval_enabled', 'change_approval_no_self', 'cloud_autosync_enabled', 'csp_report_logging', 'cve_webhook_enabled', 'debug_logging', 'dependency_link_alerts', 'drift_watch_compose', 'email_events', 'external_scheduler', 'geo_anomaly_enabled', 'iac_execute_enabled', 'image_scan_enabled', 'incident_auto_promote_enabled', 'kb_enabled', 'ldap_enabled', 'ldap_tls_verify', 'mdns_enabled', 'monitor_webhook_enabled', 'notifications_test_mode', 'offline_webhook_enabled', 'oidc_enabled', 'otlp_enabled', 'otlp_traces_enabled', 'password_breach_check', 'password_require_classes', 'port_audit_enabled', 'portal_enabled', 'portal_ticket_approval_required', 'posture_digest_enabled', 'proxmox_enabled', 'proxmox_lifecycle_enabled', 'proxmox_verify_tls', 'push_enabled', 'rdp_enabled', 'remember_me_default', 'require_agent_mtls', 'scim_enabled', 'secrets_scan_enabled', 'service_webhook_enabled', 'show_provisioning', 'siem_enabled', 'smtp_enabled', 'smtp_verify_tls', 'sso_only', 'tenancy_enforced', 'tenancy_rls', 'ticket_csat_enabled', 'trust_proxy', 'ups_auto_shutdown_enabled', 'viewers_can_ack_alerts', 'wan_watch_enabled', 'warranty_lookup_enabled', 'webhook_allow_loopback', 'webhook_block_local', 'webhook_events', 'webpush_enabled', mode='before')(_coerce_bool_loose)
        _v1 = field_validator('after_hours', 'agentless_ssh_key', 'alert_runbooks', 'apikey_default_expiry_days', 'approval_gated_kinds', 'audit_forward_host', 'audit_forward_mode', 'audit_forward_port', 'audit_forward_token', 'audit_forward_url', 'audit_hmac_auto_rotate_days', 'audit_log_retention_days', 'audit_worm_path', 'backup', 'backup_monitors', 'backup_size_anomaly_pct', 'brand_accent', 'brand_name', 'brute_force_threshold', 'brute_force_window_seconds', 'canary_files', 'cloud_accounts', 'cloud_autosync_interval', 'compliance_baseline', 'container_stale_ttl', 'csp_report_throttle_per_minute', 'ct_watch_domains', 'custom_metric_thresholds', 'cve_cache_days', 'cve_severity_filter', 'dashboard_hidden_activity_events', 'dashboard_hidden_attention_kinds', 'ddns', 'default_poll_interval', 'dependency_silence_min', 'disk_watchdog_pct', 'nic_err_alert_min', 'snmp_dead_threshold', 'temp_alert_threshold_c', 'clock_skew_threshold_ms', 'proxmox_snapshot_warn_days', 'flow_export_silence_seconds', 'flow_export_check_seconds', 'health_grade_good', 'health_grade_fair', 'health_grade_poor', 'risk_level_critical', 'risk_level_high', 'risk_level_medium', 'reliability_crit', 'reliability_high', 'reliability_medium', 'thermal_hot_c', 'thermal_crit_c', 'gpu_hot_c', 'smart_wear_warn_pct', 'smart_wear_high_pct', 'smart_realloc_crit_sectors', 'disk_predict_medium_eta_days', 'ecc_error_alert_min_delta', 'disk_forecast_crit_days', 'disk_forecast_warn_days', 'oom_recent_window_seconds', 'defender_sig_warn_days', 'cpu_pct_warn', 'cpu_pct_crit', 'patch_na_warn_count', 'container_restart_delta_threshold', 'container_restarting_min', 'attention_event_ttl_hours', 'after_hours_na_window_hours', 'snmp_cpu_warn_percent', 'snmp_cpu_crit_percent', 'agentless_ping_fail_threshold', 'ct_fail_backoff', 'github_new_issue_per_poll_cap', 'self_high_load_ratio', 'self_mem_warn_pct', 'self_online_pct_warn', 'compliance_tls_expiring_days', 'compliance_lookback_days', 'cis_disk_pct_max', 'cis_swap_pct_max', 'dmarc_pct_min', 'proxmox_backup_warn_days', 'metric_recovery_buffer', 'min_online_ttl', 'unstable_host_returns_min', 'unstable_host_window_days', 'reliability_reboot_churn_min', 'reliability_wear_high_pct', 'monitor_history_max', 'command_history_max', 'forecast_min_r2', 'forecast_ceiling_memory', 'forecast_floor_memory', 'forecast_ceiling_swap', 'forecast_floor_swap', 'forecast_ceiling_load', 'forecast_floor_load', 'reliability_realloc_growth_per_day', 'reliability_health_decline_per_day', 'cvss_band_critical', 'cvss_band_high', 'cvss_band_medium', 'inode_warn_percent', 'inode_crit_percent', 'fd_warn_percent', 'fd_crit_percent', 'conntrack_warn_percent', 'conntrack_crit_percent', 'offline_missed_polls', 'laptop_offline_grace_hours', 'resolver_failures_before_alert', 'ip_rep_confirm_scans', 'tls_warn_days', 'tls_crit_days', 'cert_file_expiring_days', 'contract_warn_days', 'contract_soon_days', 'os_eol_soon_days', 'av_sig_stale_days', 'gateway_latency_high_ms', 'battery_health_low_pct', 'enrol_rules', 'escalation', 'file_manager', 'fleet_note', 'geo_anomaly_hours', 'geoip_asn_db_path', 'geoip_db_path', 'health_alert_threshold', 'healthchecks_interval_seconds', 'healthchecks_url', 'idle_timeout_minutes', 'incident_device_threshold', 'ip_allowlist', 'ip_allowlist_enabled', 'ldap_admin_group', 'ldap_bind_dn', 'ldap_bind_password', 'ldap_required_group', 'ldap_timeout', 'ldap_url', 'ldap_user_base', 'ldap_user_filter', 'log_buffer_max_bytes_per_unit', 'log_buffer_retention_hours', 'log_ignore_patterns', 'login_banner', 'max_devices', 'max_sessions_per_user', 'metric_failures_before_alert', 'mfa_required_roles', 'monitor_interval', 'monitors', 'oidc_client_secret', 'oncall', 'online_ttl', 'otlp_endpoint', 'otlp_interval', 'otlp_token', 'otlp_traces_interval', 'password_min_length', 'patch_alert_threshold', 'patch_sla', 'portal_base_url', 'posture_digest_cadence', 'posture_digest_recipients', 'process_watches', 'proxmox_host', 'proxmox_node', 'proxmox_token_id', 'proxmox_token_secret', 'quiet_hours', 'release_key_fingerprint', 'release_pubkey', 'scim_token', 'scrub_overdue_days', 'secrets_mutes', 'secrets_scan_paths', 'self_update_command', 'server_name', 'session_ttl_long', 'session_ttl_short', 'siem_format', 'siem_token', 'siem_url', 'slo_target_percent', 'smtp_from', 'smtp_helo_name', 'smtp_host', 'smtp_password', 'smtp_port', 'smtp_recipients', 'smtp_tls', 'smtp_username', 'snapshot_stale_days', 'snmp_failures_before_alert', 'software_meters', 'sso_group_roles', 'sso_default_tenant', 'status_incident_recipients', 'status_page', 'ticket_business_hours', 'unit_flap_restarts', 'ups_critical_battery_pct', 'ups_critical_runtime_s', 'warranty_lenovo_client_id', 'warranty_provider', 'webhook_url', 'webhook_urls', 'webpush_subject', 'wol_broadcast', 'wol_port', 'health_weight_critical', 'health_weight_warning', 'health_weight_info', 'risk_weight_offline', 'risk_weight_cve_critical', 'risk_weight_cve_high', 'risk_weight_pending_updates', 'risk_weight_exposed_world', 'risk_weight_policy_violation', 'risk_weight_expiry_expired', 'risk_weight_expiry_soon', 'risk_weight_mount_issue', 'risk_weight_reboot_required', 'risk_weight_firewall_off', 'risk_weight_storage_degraded', 'risk_weight_smart_failure', 'risk_weight_kernel_outdated', 'risk_weight_failed_units', 'risk_weight_os_eol', 'risk_weight_os_eol_soon', 'risk_weight_overheating', 'risk_weight_config_drift', 'risk_weight_clock_skew', 'risk_weight_gateway_down', 'risk_weight_oom_recent', 'risk_weight_cve_kev', 'risk_weight_av_bad', 'risk_weight_image_cves', 'risk_weight_backup_stale', 'risk_weight_secrets_exposed', 'risk_weight_patch_sla_breach', 'reliability_weight_smart_failing', 'reliability_weight_realloc_growing', 'reliability_weight_pending_sectors', 'reliability_weight_wear_high', 'reliability_weight_spare_low', 'reliability_weight_ecc_uncorrectable', 'reliability_weight_ecc_correctable', 'reliability_weight_reboot_churn', 'reliability_weight_health_declining', 'reliability_weight_overheating', 'reliability_weight_oom_recent', mode='before')(_coerce_str_loose)
