...
[![Version](https://img.shields.io/badge/version-5.8.0-blue.svg)](https://github.com/tyxak/remotepower/releases)
...
| **Manage remotely** | Shell, multi-line scripts with dry-run lint, batch & scheduled runs, a real **browser SSH terminal** and **VNC** over the same tunnel, an opt-in **agent file manager** (browse / view / edit host files, no SSH), **cron & systemd-timer** management, **Proxmox** VM / LXC create plus **VMware & OpenShift** guest lifecycle (power / snapshots), and host user / key / firewall edits — all with **zero inbound ports**. |
...
| **Integrate** | 39 **homelab-app** health connectors (Pi-hole, TrueNAS, the *arr suite, …) plus a code-free **custom HTTP-probe** plugin to turn any endpoint into a signal, Prometheus / Grafana / Uptime-Kuma endpoints, inbound webhooks & syslog, and an **MCP server** so an AI client can query your fleet. |
...
- **v5.8.0 "WatchMatters" — unreleased (test)** — TLS/DANE expiry checks now run on the server's own **built-in ~6h schedule** (previously cron-only — and that cron was blind under the SQLite/Postgres backends, so scheduled scans silently never ran); no cron needed. **SNMPv3 (USM)** for agentless devices: per-device user with MD5/SHA-1/SHA-2 authentication and AES-128 privacy (noAuth/auth/authPriv), automatic engine discovery + time-window resync, write-only passwords — all existing pollers work over v3 unchanged. New **GitHub issue monitor** integration: watch repositories (`owner/repo`, several per instance) and get a `github_new_issue` alert in the Alerts inbox when a new issue is opened — PRs ignored, first poll baselines, webhook/paging off by default. No breaking changes.
