...
[![Version](https://img.shields.io/badge/version-4.1.0-blue.svg)](https://github.com/tyxak/remotepower/releases)
...
- **MCP server (read-only)** — a Model Context Protocol server exposes 12 read tools (`list_devices`, `get_device`, `get_journal`, `get_services`, `get_containers`, `get_cves`, `get_drift`, `get_recent_commands`, `get_runbook`, `get_patches`, `get_tls`, `search_devices`) so an MCP client such as Claude Desktop can answer questions about your fleet. Deliberately read-only — there are no write/exec tools, so the assistant can inspect but never change anything. See [docs/mcp.md](docs/mcp.md).
