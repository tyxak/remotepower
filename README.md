...
[![Version](https://img.shields.io/badge/version-5.4.1-blue.svg)](https://github.com/tyxak/remotepower/releases)
...
- **v5.4.1 — RackMatters (follow-up)** — AV/rootkit scan **warnings** (rkhunter `[Warning]` lines, stale ClamAV DB) now raise a first-class **`av_warning`** alert instead of only a "Needs attention" card; ticket **attachments** (extract + download/preview inbound, send outbound ≤15 MB); a loop-safe ticket **auto-reply** acknowledgement; a one-click **"View email thread"** printable window; the **Billing** page is now opt-in under **Settings → Advanced** (logging hours / the Timesheet stay on); and a small ticket glyph marks ticketed hosts on the Devices page. No breaking changes.
