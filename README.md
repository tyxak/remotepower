...
[![Version](https://img.shields.io/badge/version-3.14.0-blue.svg)](https://github.com/tyxak/remotepower/releases)
...
- **Operator quality-of-life batch** *(v3.14)* — **per-account sidebar favorites** (star any entry to pin under "Main"; synced across devices via `/api/me`); a **per-container stale-image badge** in the drawer (same digest join as the fleet Image Updates page); a fleet **Thermal "hottest hosts"** page; **CVE prioritization** with CISA **KEV** + **EPSS** (rank by exploited-in-the-wild, not just CVSS); **active session management** (list & revoke your signed-in sessions); **saved & shareable Devices views**; **SSD/NVMe wear** + projected end-of-life; **GPU monitoring**; a **local certificate-file** expiry inventory; a **local account audit** (extra UID 0 / stale / locked); and **power scheduling** (scheduled Suspend / Wake-on-LAN). Plus a **predictive Disk-health** page (SMART-trend failure ETA), a **Power** page (UPS via NUT/apcupsd + energy-cost estimate), an **SSH-key audit** (fleet authorized_keys with SHA256 fingerprints), on-demand **container log** fetch, **global omnisearch** (palette also searches alerts + CVEs), and a **custom report builder** (pick sections, JSON/CSV, per-report schedule). Most of it surfaces data the agent already reports. See **[docs/v3.14.0.md](docs/v3.14.0.md)**.
