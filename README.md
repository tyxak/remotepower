...
- **Favorites, stale-image badges & fleet thermals** *(v3.14)* — **star** any sidebar entry to pin it under "Main"; favorites are **per-account** and follow you across devices (`/api/me` + `/api/favorites`, with localStorage as a fast-paint fallback). The device-drawer **Containers** table gains a **stale-image badge** (an *update* pill when a container's running image is behind the registry, using the same digest join as the fleet Image Updates page). And a new **Thermal** page rolls up the **hottest hosts** fleet-wide — one row per host with its hottest CPU/chipset/disk sensor, sorted hottest-first — over the temps the agent already reports. See **[docs/v3.14.0.md](docs/v3.14.0.md)**.
