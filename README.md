...
- **v5.8.0 — unreleased (test)** — TLS/DANE expiry checks now run on the server's own **built-in ~6h schedule** (previously cron-only — and that cron was blind under the SQLite/Postgres backends, so scheduled scans silently never ran); no cron needed. **SNMPv3 (USM)** for agentless devices: per-device user with MD5/SHA-1/SHA-2 authentication and AES-128 privacy (noAuth/auth/authPriv), automatic engine discovery + time-window resync, write-only passwords — all existing pollers work over v3 unchanged. New **GitHub issue monitor** integration: watch repositories (`owner/repo`, several per instance) and get a `github_new_issue` alert in the Alerts inbox when a new issue is opened — PRs ignored, first poll baselines, webhook/paging off by default. No breaking changes.
