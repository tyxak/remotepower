...
- **InterfaceMatters** *(v4.6)* — a **visual-identity** release, **no markup/nav/functional changes**. A distinctive **Industrial** interface ("New UI") becomes the default — a warm **graphite/steel** palette (keeping the RemotePower **blue** accent), corner-tick panels, dashed technical rules, mono uppercase eyebrow labels and tabular figures — replacing the generic slate-navy template look. A one-click **New UI / Old UI** toggle (**Settings → Interface**, and **My Account → Appearance**) restores the classic look; the choice is per-browser and applies instantly. Pure CSS + a `data-ui` attribute, reusing the self-hosted JetBrains Mono — fully **CSP-safe**, no external fonts. See **[docs/v4.6.0.md](docs/v4.6.0.md)**.
