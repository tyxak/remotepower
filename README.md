...
- **Bind it together — round four** *(v3.13)* — surfaces host signals the agent already collected but the UI never showed: per-device **recent logins & source IPs**, a failed-first **systemd timer** inventory, this host's own **ZFS/mdadm/btrfs storage health**, **bind address + world/LAN/local scope** on the drawer's listening-ports card, the **firewall ruleset fingerprint**, an active **brute-force lockout** badge, and Disk/Swap pressure pills. Plus every panel now caps at ~15 rows and scrolls; static assets cache immutably; and a security pass (sandboxed SCAP reports, OIDC id_token expiry/issuer/audience checks, syslog-forward DNS-rebinding fix). See **[CHANGELOG.md](CHANGELOG.md)**.
- **Operator quality-of-life batch** *(v4.0)* — **per-account sidebar favorites** (star any entry to pin under "Main"; synced across devices via `/api/me`); a **per-container stale-image badge** in the drawer (same digest join as the fleet Image Updates page); a fleet **Thermal "hottest hosts"** page; **CVE prioritization** with CISA **KEV** + **EPSS** (rank by exploited-in-the-wild, not just CVSS); **active session management** (list & revoke your signed-in sessions); **saved & shareable Devices views**; **SSD/NVMe wear** + projected end-of-life; **GPU monitoring**; a **local certificate-file** expiry inventory; a **local account audit** (extra UID 0 / stale / locked); and **power scheduling** (scheduled Suspend / Wake-on-LAN). Plus a **Predictive-health** page (SMART-trend disk-failure ETA + frequently-restarting hosts), a **Power** page (UPS via NUT/apcupsd + energy-cost estimate), an **SSH-key audit** (fleet authorized_keys with SHA256 fingerprints), on-demand **container log** fetch, **global omnisearch** (palette also searches alerts + CVEs), a **custom report builder** (pick sections, JSON/CSV, per-report schedule), **container images in SBOMs**, a **fleet drift-enforcement policy** by tag/group, **alerts** for the new posture signals (predicted disk failure, UPS-on-battery, expiring cert files, rogue UID-0), **Prometheus metrics push** to a Pushgateway, a **customizable dashboard** (show/hide & reorder Home widgets, saved per account), **GitOps** (sync drift-config profiles from a Git manifest — config-as-code, dry-run preview), an **interface language** switch (English / Mandarin / Hindi / Spanish / Arabic, saved per account), and **change-approval** coverage extended to reboot/shutdown/update/upgrade/uninstall/container actions. Most of it surfaces data the agent already reports. See **[CHANGELOG.md](CHANGELOG.md)**.

- **VisualMatters** *(v4.1)* — binds the telemetry the agent already reports into first-class views. A **CheckMK-style per-host Checks** page (under Monitoring) shows every signal on every host as OK / WARN / CRIT / UNKNOWN with output — sortable, filterable, per-check muteable, with *Hide muted* / *Hide unmonitored* on by default; **custom checks** (server-side process/port + agent-side file/job/log) assignable to host/tag/group, and your custom scripts surface as rows. More **active monitors**: DNS (with expected-address), ICMP latency + packet-loss, HTTP status + latency-SLA assertions, a credential-less **DB-liveness** probe (PostgreSQL/MySQL/Redis), and tag/group target fan-out. A **composable dashboard** — a 66-widget catalog you resize / reorder / show-hide / reset / align / share, with Upcoming, Tickets and actionable Alerts widgets. A **host-grouped alert inbox** that folds a host-down storm under its `device_offline` root cause. Plus a **TLS 1.2 floor** on the satellite (both hops), the agent, and server outbound HTTPS. Independently scanned (wapiti, nikto, nuclei, bandit, OWASP ZAP) — clean. See **[CHANGELOG.md](CHANGELOG.md)**.

- **5ecur1tyM4tter5** *(v4.2)* — security and integrations: making the fleet **defensible**, not just visible. **Authorized vulnerability scanning** (the *Pentest* page) scans the hosts and websites you own with industry tools (passive: nuclei / nikto / nmap; active: OWASP ZAP / wapiti; on-host: lynis) — targets are authorization-gated (enrolled hosts, or domains you prove you own via an ACME-style DNS-TXT / `.well-known` check), the toolchain runs on a hardened **scanner satellite**, the active tier is gated behind an authorization attestation + maintenance window, and scans can be **scheduled**. **Passkeys** (WebAuthn) bring phishing-resistant, passwordless sign-in that satisfies MFA; **SAML 2.0 SSO** adds enterprise IdP login (Okta / Entra / OneLogin / Ping / ADFS) alongside the existing OIDC / LDAP. A **tamper-evident** (hash-chained) audit log with one-click integrity verify and a protected clear; plus account guardrails — **enforce MFA per role**, **cap concurrent sessions**, **default API-key expiry**, and a graded **security-posture self-check**. Every control is opt-in. See **[CHANGELOG.md](CHANGELOG.md)**.
- **ImprovementMatters** *(v4.3)* — a refinement release, **no breaking changes**: sharpens what's already there. **Faster on bigger fleets** — on the SQLite / PostgreSQL backends the single-device endpoints (per-host Checks, per-device CVE detail, the heartbeat's host-config + watched-file lookups, and the firewall / compose / RouterOS / OPNsense device actions) read **one row** instead of reconstructing the whole fleet per request. **Self-observability** — download the gzipped **audit-log archive** from the Audit page, and see a last-ran + staleness badge for cadence jobs (monitors, KEV/EPSS refresh, scheduled scans) on **Server status**. **UX** — each security-posture warning links straight to the Settings section that fixes it, and tables share one skeleton loading state. Plus **regression guardrails** (tests) that catch a check reading an unpersisted sysinfo field, or a webhook event that half-registers, at commit time. See **[CHANGELOG.md](CHANGELOG.md)**.
- **FortifyMatters** *(v4.4)* — a security-hardening + bind-it-together release, **no breaking changes**. A full server + agent audit, **independently pentested clean** (wapiti / nikto / nuclei / bandit / OWASP ZAP), fixed a **critical** admin-gate escalation (a custom operator role could reach admin-only endpoints), scope-guarded the mitigation diagnostic routes, `shlex`-quoted the drift/ACME command builders, added an SSRF guard to the RouterOS integration, made `/api/metrics` un-knock-over-able, closed an agent `/tmp` symlink-clobber, and brought the **Windows & macOS agents to HTTPS + TLS-1.2 parity** with Linux. Plus **bind-it-together** (login timestamps and an accurate clock-skew verdict in the device drawer), **faster heartbeats** (single-row reads; the 15s fleet-checks cache now actually caches), and box-overflow + typography polish. See **[CHANGELOG.md](CHANGELOG.md)**.
- **DocumentationMatters** *(v4.4.1)* — a documentation + triage release, **no code-behaviour changes**. Triaged the open **CodeQL** code-scanning alerts (XSS, SSRF, weak-hash, clear-text logging/storage) — **all 13 confirmed false positives**, each mapped to the feature that already covers it (the `escHtml`/`escAttr` sanitizers, the same-origin API client, the HMAC-SHA256 audit chain), and silenced the two MD5 cache-key fingerprints with `usedforsecurity=False`. Plus a documentation-coverage pass across the in-app docs, README, Manual, and per-version notes. See **[docs/v4.4.1.md](docs/v4.4.1.md)** and **[docs/security-review-4.4.1.md](docs/security-review-4.4.1.md)**.
- **TrustMatters** *(v4.5)* — **TLS onboarding**, **no breaking changes**. A one-command self-signed **CA** (`make tls-selfsigned HOST=…` / `tools/gen-ca.sh`) for internal/airgapped instances that can't use a real cert — agents trust the *CA*, so the server cert renews (`make tls-renew`) without touching any client. **Fingerprint-verified rollout**: `install-client.sh --ca-fingerprint <sha256>` (and the macOS/Windows installers) fetch the CA over HTTP and refuse to trust it on a mismatch, then wire it in via `RP_CA_BUNDLE`. **Docker opt-in TLS** (`RP_TLS_SELFSIGNED=1`) generates a CA+leaf into the data volume and serves HTTPS on first boot. Switching self-signed → a real cert is a **server-only** change (agents trust the system roots *and* the CA simultaneously). A real cert is still recommended. See **[docs/tls-selfsigned.md](docs/tls-selfsigned.md)** and **[docs/v4.5.0.md](docs/v4.5.0.md)**.
- **RepellantMatters** *(v4.6)* — a **visual-identity** release paired with a **project-wide reliability, security and performance polish pass**. A distinctive **Industrial** interface ("New UI") becomes the default — a warm **graphite/steel** palette (keeping the RemotePower **blue** accent), corner-tick panels, dashed technical rules, mono uppercase eyebrow labels and tabular figures, with the **sidebar/nav in self-hosted IBM Plex Mono** and the rest of the UI in the familiar **Inter** — replacing the generic slate-navy template look. A one-click **New UI / Old UI** toggle (**Settings → Interface**, and **My Account → Appearance**) restores the classic look; the choice is per-browser and applies instantly. Pure CSS + a `data-ui` attribute — fully **CSP-safe**, no external fonts. Alongside the skin: a dedicated **Admin** sidebar group with alphabetized menus, more device telemetry surfaced in the drawer, an **independently pentested-clean** security pass (wapiti, nikto, nuclei, bandit, OWASP ZAP), and dashboard/agent performance work. See **[docs/v4.6.0.md](docs/v4.6.0.md)**.
- **v4.6.1** — a **stability + hardening patch** on v4.6.0, **no breaking changes**. Fixes two issues that only surface under the optional **SCGI prefork worker**: agentless **ICMP** devices flapped offline (the worker's `NoNewPrivileges` blocked `ping`'s capability elevation — now granted `CAP_NET_RAW` ambiently), and a forked worker child inherited the parent's **Postgres** connection (*consuming input failed: EOF detected* — now PID-guarded and reconnected, like the SQLite backend). Plus a **ReDoS** fix in `_valid_tls_host` (label-by-label validation instead of a catastrophically-backtracking regex), a defence-in-depth **XSS** coercion on the package update badge, and a **UI** fix so the page subtitle no longer flashes as raw text on reload. See **[docs/v4.6.1.md](docs/v4.6.1.md)**.
- **IntegrationsMatters** *(v4.7)* — a **reach-outward** release, **no breaking changes**. **Homelab/fleet software integrations**: a read-only server-side subsystem polls **26** popular self-hosted apps for health and folds it into Alerts + the dashboard — Pi-hole, AdGuard, TrueNAS, UniFi, Home Assistant, Proxmox Backup Server, Kubernetes/k3s, vCenter/ESXi, Unraid, Traefik, Nginx Proxy Manager, Caddy, Netdata, Grafana, Uptime Kuma, Jellyfin, Plex, Nextcloud, the download clients (qBittorrent/Transmission/Deluge/SABnzbd/NZBGet), the *arr suite (one **Servarr** connector for Sonarr/Radarr/Prowlarr/Lidarr) + Bazarr, and Overseerr/Jellyseerr. Configure under **Settings → Integrations**; an unhealthy/unreachable target raises an auto-resolving `integration_down` alert. Every call is **SSRF-guarded** (loopback/link-local/metadata refused, peer re-checked at connect time, no redirects), credentials are redacted and the raw URL is admin-only, and a **Show Homelab software** switch hides the whole feature for enterprise. Plus a **containerized agent**: run the Linux agent as a **container** that monitors a Docker host with no install (*Enroll device → Generate Docker compose*), published multi-arch at **`ghcr.io/tyxak/remotepower-agent`** — standard caps, no `--privileged`. Plus a **Fleet GPU** page (Monitoring → GPUs) showing every GPU across the fleet (**NVIDIA + AMD**) with per-GPU utilisation / VRAM / temperature / power / fan, hottest-busiest first, **temperature + utilisation trend sparklines** on each card, and **thermal alerting** — a GPU at or above the temperature threshold (default 85 °C, configurable) raises the standard high-temperature alert and auto-resolves when it cools. See **[docs/v4.7.0.md](docs/v4.7.0.md)**, **[docs/integrations.md](docs/integrations.md)** and **[docs/docker-agent.md](docs/docker-agent.md)**.
...
**[docs/security-review-4.4.1.md](docs/security-review-4.4.1.md)** (a triage of
the open CodeQL code-scanning alerts — all false positives), preceded by the
**[4.4.0](docs/security-review-4.4.0.md)** full server + agent audit
(independently pentested clean) and the 4.3.0 review. It is
