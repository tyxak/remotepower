...
- **v5.8.0 — unreleased (test)** — TLS/DANE expiry checks now run on the server's own **built-in ~6h schedule** (previously cron-only — and that cron was blind under the SQLite/Postgres backends, so scheduled scans silently never ran); no cron needed. New **GitHub issue monitor** integration: watch repositories (`owner/repo`, several per instance) and get a `github_new_issue` alert in the Alerts inbox when a new issue is opened — PRs ignored, first poll baselines, webhook/paging off by default. No breaking changes.
