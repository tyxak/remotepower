...
if [[ "${1:-}" == "--uninstall" ]]; then
  ...
  systemctl disable --now remotepower-demo-seed.timer remotepower-wsgi-demo remotepower-scheduler-demo 2>/dev/null || true
  rm -f /etc/systemd/system/remotepower-{wsgi-demo,scheduler-demo,demo-seed}.service /etc/systemd/system/remotepower-demo-seed.timer
  systemctl daemon-reload 2>/dev/null || true
  echo "  stopped + removed the demo app-server, scheduler and re-seed units"
...
# Out-of-band scheduler (below) owns the ~33 maintenance sweeps for this data
# dir — without this the request path re-runs every "is it due?" check on
# EVERY hit, which on a networked Postgres backend measured ~0.68s -> ~0.027s
# per request (25x) once moved out-of-band (see remotepower-scheduler.service).
# Found live: the demo was left running the request-path cadence and was
...
ExecStart=/usr/bin/gunicorn --workers 2 --threads 4 --timeout 120 \\
          --keep-alive 75 --max-requests 1000 --max-requests-jitter 100 --bind 127.0.0.1:$DEMO_PORT wsgi:application
Restart=always
RestartSec=2
...
SCHEDULER_UNIT_PATH="/etc/systemd/system/remotepower-scheduler-demo.service"
SCHEDULER_UNIT_BODY=$(cat <<EOF
# RemotePower demo out-of-band maintenance scheduler — auto-generated by
# install-demo.sh. Runs the ~33 run_*_if_due sweeps for \$DEMO_DATA_DIR from a
# dedicated process, paired with RP_EXTERNAL_SCHEDULER=1 on
# remotepower-wsgi-demo.service above (see server/conf/remotepower-scheduler.service
# for the full rationale — same pattern, just pointed at the demo's own data dir).
[Unit]
Description=RemotePower demo out-of-band maintenance scheduler
After=network.target
...
ExecStart=/usr/bin/python3 $PROD_HTDOCS/cgi-bin/scheduler.py
Restart=always
RestartSec=5
NoNewPrivileges=true
ProtectHome=true
...
SEED_UNIT_PATH="/etc/systemd/system/remotepower-demo-seed.service"
SEED_UNIT_BODY=$(cat <<EOF
# RemotePower demo re-seed — auto-generated by install-demo.sh.
# The seed is a SNAPSHOT: last_seen, checked_at and the cadence markers are all
# stamped at "now". Left alone the fleet ages out of its own offline window and
# the demo turns into a wall of dead hosts, which is why the timer beside this
# unit re-runs it. Re-seeding is idempotent (deterministic ids, same numbers per
# device), so what moves is the freshness, not the content.
[Unit]
Description=RemotePower demo data re-seed
After=network.target
...
User=$CGI_USER
Group=$CGI_GROUP
ExecStart=/usr/bin/python3 $SEED_SCRIPT --data-dir $DEMO_DATA_DIR --apply --quiet
NoNewPrivileges=true
# read-only rather than true: ExecStart runs \$SEED_SCRIPT from wherever this
# installer was invoked, which can be a home directory. ProtectHome=true hides
# it and the unit fails with a bare "No such file", which reads as a missing
# script rather than a sandbox.
...
SEED_TIMER_PATH="/etc/systemd/system/remotepower-demo-seed.timer"
SEED_TIMER_BODY=$(cat <<EOF
# Re-seed every 2 minutes. The seeded fleet's own offline threshold is
...
if [[ "$DRY_RUN" -eq 1 ]]; then
  ...
  echo "DRY-RUN: would write to $SCHEDULER_UNIT_PATH:"
  echo "$SCHEDULER_UNIT_BODY" | sed 's/^/  | /'
  echo "DRY-RUN: would write to $SEED_UNIT_PATH:"
  echo "$SEED_UNIT_BODY" | sed 's/^/  | /'
  echo "DRY-RUN: would write to $SEED_TIMER_PATH:"
  echo "$SEED_TIMER_BODY" | sed 's/^/  | /'
  echo "DRY-RUN: would run: systemctl daemon-reload && systemctl enable --now remotepower-wsgi-demo remotepower-scheduler-demo remotepower-demo-seed.timer"
else
  ...
  printf '%s\n' "$UNIT_BODY" > "$UNIT_PATH"
  chmod 644 "$UNIT_PATH"
  printf '%s\n' "$SCHEDULER_UNIT_BODY" > "$SCHEDULER_UNIT_PATH"
  chmod 644 "$SCHEDULER_UNIT_PATH"
  systemctl daemon-reload
  systemctl enable --now remotepower-scheduler-demo \
    || die "remotepower-scheduler-demo failed to start — check: systemctl status remotepower-scheduler-demo"
  ...
  printf '%s\n' "$SEED_UNIT_BODY" > "$SEED_UNIT_PATH"
  chmod 644 "$SEED_UNIT_PATH"
  printf '%s\n' "$SEED_TIMER_BODY" > "$SEED_TIMER_PATH"
  chmod 644 "$SEED_TIMER_PATH"
  systemctl daemon-reload
  systemctl enable --now remotepower-demo-seed.timer \
    || die "remotepower-demo-seed.timer failed to start — check: systemctl status remotepower-demo-seed.timer"
fi
...
$IPV6_LISTEN
    ...
    # PWA service worker + manifest are fetched from the site ROOT (not
    # /static/) and don't match the generic \\.html\$/= / rules below — the
    # catch-all 404 was swallowing both, breaking service-worker registration
    # on the demo (found live: "A bad HTTP response code (404) was received
    # when fetching the script" for sw.js).
    location = /sw.js {
        expires -1;
...
To remove:
  sudo systemctl disable --now remotepower-wsgi-demo remotepower-scheduler-demo
  sudo rm $UNIT_PATH $SCHEDULER_UNIT_PATH
  sudo rm $NGINX_CONF
