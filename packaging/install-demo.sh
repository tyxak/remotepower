...
if [[ "${1:-}" == "--uninstall" ]]; then
  ...
  systemctl disable --now remotepower-wsgi-demo remotepower-scheduler-demo 2>/dev/null || true
  rm -f /etc/systemd/system/remotepower-wsgi-demo.service /etc/systemd/system/remotepower-scheduler-demo.service
  systemctl daemon-reload 2>/dev/null || true
  echo "  stopped + removed the demo app-server and scheduler units"
...
if [[ "$DRY_RUN" -eq 1 ]]; then
  ...
  echo "DRY-RUN: would run: systemctl daemon-reload && systemctl enable --now remotepower-wsgi-demo remotepower-scheduler-demo"
else
