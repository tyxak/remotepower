...
def build_devices() -> dict:
    ...
    for dev in FAKE_DEVICES:
        ...
        rec = {
            ...
            'monitored':   True,
            'poll_interval': 60,
            'version':     '2.1.6' if not dev['agentless'] else None,
            'hostname':    dev['name'],
        }
        if not dev['agentless']:
            ...
            # v2.1.6+: compose_projects for demo realism. Only devices that
...
def build_history() -> list:
    ...
    return [
        ...
        {'ts': now() - 86400 * 7, 'actor': 'demo', 'device': 'all',    'action': 'agent_update',     'detail': 'fleet-wide agent update to v2.1.6'},
    ]
...
def build_scripts() -> dict:
    """v2.1.6 Script Library demo content. Five realistic operations
